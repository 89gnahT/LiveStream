...
class MediaRecorder: NSObject {
    ...
    func startRecording(mediaType : MediaWriterFileType, videoCodecType: AVVideoCodecType, outputSize : CGSize, transformRotationDegreeAngle : Double = 0){
        recoderQueue.async {
            if self.status == .unknow || self.status == .stopped{
                // Start recording
                self.status = .recording
                
                self.mediaWriter = MediaFIleWriterStore.CreateVideoWriter(mediaType: mediaType, videoCodecType: videoCodecType, outputSize: outputSize, transformRotationDegreeAngle: transformRotationDegreeAngle)
