...
class MediaFileWriter: NSObject {
    ...
    func createNewVideoInput(videoCodecType: AVVideoCodecType, outputSize : CGSize, transformRotationDegreeAngle : Double = 0) -> Bool{
        var success = false
        ...
        writerQueue.sync {
            if status == .unknown{
                ...
                videoAssetWriterInput?.transform = CGAffineTransform(rotationAngle: CGFloat(Double.pi * transformRotationDegreeAngle / 180)) ;
